...
data Model d where
  ...
  RotateA :: (V d Radian) -> (Model d) -> Model d
  RotateV :: Radian -> (V d Double) -> (Model d) -> Model d
  Scale :: (V d Double) -> (Model d) -> Model d
  Resize :: (V d Double) -> (Model d) -> Model d
  Mirror :: (V3 Double) -> (Model 'Three) -> Model 'Three
...
block :: (Pretty (V d Double), Pretty (V d Radian)) => [Model d] -> Doc ann
...
instance (Pretty (V d Double), Pretty (V d Radian)) => Pretty (Model d) where
  ...
  pretty (Square r c) =
    "square" <> tupled (pretty r : [center c]) <> ";"
  pretty (Rectangle r c) =
    "square" <> tupled (pretty r : [center c]) <> ";"
  pretty (Polygon vs [] c) =
    "polygon" <> tupled (list (fmap pretty vs) : named' "convexity" c) <> ";"
  pretty (Polygon vs [p] c) =
    "polygon" <> tupled (list (fmap pretty vs) : named "paths" p : named' "convexity" c) <> ";"
  pretty (Polygon vs ps c) =
    "polygon" <> tupled (list (fmap pretty vs) : named "paths" ps : named' "convexity" c) <> ";"
  ...
  pretty (Cylinder2 h r1 r2 c f) =
    "cylinder" <> tupled ((named "h" h) : (named "r1" r1) : (named "r2" r2) : center c : ppFacets f) <> ";"
  ...
  pretty (OffsetDelta d c m) =
    "offset" <> tupled [named "delta" d, named "chamfer" (ppBool c)] <+> block [m]
